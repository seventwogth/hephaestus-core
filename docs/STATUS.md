...
- Оркестратор умеет создавать проект, сохранять начальные артефакты, формировать `PLAN.json` и запускать один агентный этап через `ModelProvider`.
...
1. Добавить генерацию backend-модулей Go поверх шаблона.
2. Добавить генерацию frontend-страниц и API-клиента поверх шаблона.
