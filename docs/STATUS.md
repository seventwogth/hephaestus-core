...
- Усилен `project-sandbox`: команды запускаются с минимальным env без host secrets, workspace-local HOME/cache/tmp, timeout telemetry, kill escalation, output limit, realpath-проверками symlink escape и запретом hardlinked files; sandbox failure modes попадают в `GENERATION_REPORT.json`.
...
- Phase 1 sandbox hardening начат: file/path boundary, command env isolation, runtime cache cleanup, timeout escalation, output limit и generation report failure summary покрыты тестами, но ещё нет container workspace, CPU/memory/disk/process limits и network policy по стадиям.
...
1. Продолжить Phase 1 production roadmap: добавить container workspace, CPU/memory/disk/process limits и network policy по стадиям.
