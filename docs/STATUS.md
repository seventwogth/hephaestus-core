...
- Оркестратор умеет создавать проект, сохранять начальные артефакты, формировать `PLAN.json`, генерировать SQL-миграции, генерировать Go backend-маршруты, генерировать frontend-страницу и запускать один агентный этап через `ModelProvider`.
...
1. Реализовать ограниченный цикл исправлений по `REVIEW.md`.
2. Подключить Telegram-бота как пользовательский интерфейс MVP.
3. Расширить генерацию БД за пределы одной основной сущности: связи, индексы и типизированные поля.
