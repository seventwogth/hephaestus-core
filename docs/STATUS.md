...
- Усилен `project-sandbox`: команды запускаются с минимальным env без host secrets, workspace-local HOME/cache/tmp, timeout telemetry, kill escalation, output limit, realpath-проверками symlink escape и запретом hardlinked files.
...
- Phase 1 sandbox hardening начат: file/path boundary, command env isolation, runtime cache cleanup, timeout escalation и output limit покрыты тестами, но ещё нет container workspace, CPU/memory/disk/process limits и network policy по стадиям.
...
1. Продолжить Phase 1 production roadmap: добавить container workspace, CPU/memory/disk/process limits, network policy и sandbox failure reporting в generation report.
