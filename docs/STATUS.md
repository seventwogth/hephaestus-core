...
- Telegram-бот пока хранит сессии в памяти процесса и только сохраняет выбранную модель в `MODEL_SELECTION.json`; реального переключения между внешними model-provider implementations пока нет.

## Следующие крупные шаги

1. Расширить генерацию БД за пределы одной основной сущности: связи, индексы и типизированные поля.
2. Добавить реальный провайдер моделей поверх `ModelProvider`, чтобы fixer loop, Telegram-бот и агентные этапы работали вне stub-режима.
3. Добавить персистентное хранение Telegram-сессий и очереди проектов между перезапусками процесса.
