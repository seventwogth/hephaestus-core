...
- Оркестратор умеет создавать проект, сохранять начальные артефакты, формировать `PLAN.json`, генерировать базовые Go backend-маршруты, генерировать frontend-страницу и запускать один агентный этап через `ModelProvider`.
...
1. Добавить постоянный слой БД для Go backend: миграции и SQL-доступ.
2. Реализовать ограниченный цикл исправлений по `REVIEW.md`.
3. Подключить Telegram-бота как пользовательский интерфейс MVP.
