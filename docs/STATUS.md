...
- Telegram-бот теперь переживает рестарты по session/queue state, но job runner всё ещё живёт в том же polling-процессе, а не вынесен в отдельный daemon/service.
...
1. Вынести job runner в отдельный daemon/service на хост-машине, чтобы Telegram polling UI и выполнение очереди можно было масштабировать и перезапускать независимо.
