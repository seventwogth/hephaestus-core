...
1. Добавить реальный провайдер моделей поверх `ModelProvider`, чтобы fixer loop, Telegram-бот и агентные этапы работали вне stub-режима.
2. Добавить персистентное хранение Telegram-сессий и очереди проектов между перезапусками процесса.
