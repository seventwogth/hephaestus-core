...
- Усилен `project-sandbox`: команды запускаются с минимальным env без host secrets, workspace-local HOME/cache/tmp, timeout telemetry, realpath-проверками symlink escape и запретом hardlinked files.
...
- Phase 1 sandbox hardening начат: file/path boundary и command env isolation покрыты тестами, но ещё нет container workspace, CPU/memory/disk/process limits и network policy по стадиям.
...
1. Продолжить Phase 1 production roadmap: добавить container workspace, resource limits, network policy и sandbox failure reporting в generation report.
